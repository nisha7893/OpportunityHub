...
function SaveButton({ jobId, dark, isLoggedIn, savedIds, onToggle }: { jobId: string; dark: boolean; isLoggedIn: boolean; savedIds: string[]; onToggle: (id: string) => void }) {
  ...
  return (
    <button
      onClick={handleClick}
      style={{ color: isSaved ? "#f59e0b" : dark ? "#6b7280" : "#9ca3af" }}
...
function Modal({ opp, onClose, dark }: { opp: Opportunity; onClose: () => void; dark: boolean }) {
  const deadline = new Date(opp.deadline).toLocaleDateString("en-IN", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  ...
  return (
   <div
    ...
    >
      <div
        style={{ backgroundColor: cardBg, color: textMain, animation: "modalIn 0.25s ease" }}
        className="rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 flex flex-col gap-4"
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <span
              style={{
                backgroundColor: opp.type === "Internship" ? "#99f6e4" : "#fde68a",
                color: opp.type === "Internship" ? "#0f766e" : "#92400e",
              }}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {opp.skills.map((skill) => (
                <span
                  key={skill}
                  style={{ backgroundColor: dark ? "#272b33" : "#f3f4f6", color: textSub }}
        ...
        <div>
          ...
          <p style={{ color: textSub }} className="text-sm leading-relaxed">
            {opp.description.replace(/<[^>]*>/g, "").substring(0, 500)}
...
function JobCard({ opp, onClick, dark, isLoggedIn, savedIds, onToggleSave }: { opp: Opportunity; onClick: () => void; dark: boolean; isLoggedIn: boolean; savedIds: string[]; onToggleSave: (id: string) => void }) {
  ...
  return (
    <div
      ...
      className="border rounded-2xl p-5 flex flex-col gap-3 cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all duration-200"
    >
      <div className="flex items-start justify-between">
        <span
          style={{
            backgroundColor: isInternship ? (dark ? "#0f3a36" : "#ccfbf1") : (dark ? "#3a2e0f" : "#fef3c7"),
            color: isInternship ? "#2dd4bf" : "#f59e0b",
          }}
          className="text-xs font-semibold uppercase px-2.5 py-1 rounded-full"
        >
          {opp.type}
    ...
    <div style={{ backgroundColor: bg, minHeight: "100vh" }}>
      <header style={{ backgroundColor: headerBg, borderColor: headerBorder }} className="border-b">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Logo />
            <h1 style={{ color: textMain }} className="text-xl font-bold">
              Opportunity<span style={{ color: accent }}>Hub</span>
            </h1>
          </div>
          ...
          <div className="flex items-center gap-3">
            {user ? (
              <>
                ...
                <button
                  ...
                  style={{
                    backgroundColor: darkMode ? "#1c1f26" : "#f3f4f6",
                    color: textMain,
                    borderColor: darkMode ? "#2a2e37" : "#e5e7eb",
                  }}
            ...
            <button
              ...
              style={{
                backgroundColor: darkMode ? "#1c1f26" : "#0f172a",
                color: darkMode ? accent : "#ffffff",
              }}
              className="px-4 py-2 rounded-full text-sm font-semibold hover:scale-105 active:scale-95 transition-transform"
            >
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center mb-12">
          <div>
            <h2 style={{ color: textMain }} className="text-4xl sm:text-5xl font-extrabold leading-tight mb-4">
              Got potential?
              <br />
              Meet opportunity.
            </h2>
            <p style={{ color: textSub }} className="text-base mb-6">
              Internships and jobs from top companies, synced automatically every 6 hours.
            </p>
            ...
            <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-2 mb-3">
              <input
                ...
                style={{
                  ...
                  borderColor: darkMode ? "#2a2e37" : "#d1d5db",
                  color: textMain,
                }}
                className="flex-1 border rounded-full px-5 py-3 text-sm outline-none"
              />
              <button
                type="submit"
                style={{ backgroundColor: "#0f766e" }}
                className="text-white font-semibold px-6 py-3 rounded-full text-sm whitespace-nowrap"
              >
                Search
              </button>
            </form>
            ...
            <p style={{ color: textSub }} className="text-xs">
              Popular: Software Intern, Data Analyst, SDE
            </p>
          </div>
          ...
          <div className="relative flex justify-center lg:justify-end">
            ...
            <span
              style={{ backgroundColor: darkMode ? "#1c1f26" : "#ffffff", color: textMain }}
              className="absolute -top-2 right-2 sm:right-0 text-xs font-semibold px-3 py-1.5 rounded-full shadow"
            >
              {opportunities.length} live roles
            </span>

            <span
              style={{ backgroundColor: darkMode ? "#1c1f26" : "#ffffff", color: accent }}
              className="absolute bottom-4 -left-2 sm:left-4 text-xs font-semibold px-3 py-1.5 rounded-full shadow"
            >
              Synced 2h ago
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-10">
          <div
            style={{ backgroundColor: darkMode ? "#0f3a36" : "#ccfbf1" }}
            className="rounded-2xl p-6 flex flex-col gap-3"
          >
            <h3 style={{ color: darkMode ? "#ffffff" : "#134e4a" }} className="text-xl font-bold">
              Internships
            </h3>
            <p style={{ color: darkMode ? "#5eead4" : "#0f766e" }} className="text-sm">
              Find roles for students and recent graduates.
            </p>
            <button
              onClick={() => setTypeFilter("Internship")}
              style={{ backgroundColor: "#0f766e" }}
              className="w-fit text-white text-sm font-semibold px-5 py-2.5 rounded-full mt-1"
            >
              Browse internships
            </button>
          </div>
          ...
          <div
            style={{ backgroundColor: darkMode ? "#3a2e0f" : "#fef3c7" }}
            className="rounded-2xl p-6 flex flex-col gap-3"
          >
            <h3 style={{ color: darkMode ? "#ffffff" : "#78350f" }} className="text-xl font-bold">
              Full-time jobs
            </h3>
            <p style={{ color: darkMode ? "#fbbf24" : "#b45309" }} className="text-sm">
              Explore engineering and tech roles at top companies.
            </p>
            <button
        ...
        <div className="flex gap-3 mb-6">
          {["", "Internship", "Job"].map((t) => (
            <button
              ...
              style={{
                backgroundColor: typeFilter === t ? "#0f766e" : darkMode ? "#1c1f26" : "#ffffff",
                color: typeFilter === t ? "#ffffff" : textMain,
                borderColor: darkMode ? "#2a2e37" : "#e5e7eb",
              }}
        ...
        {!loading && opportunities.length === 0 && (
          <p style={{ color: textSub }} className="text-center py-20">No opportunities found.</p>
        )}
